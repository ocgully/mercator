...
def render(doc: dict) -> str:
    ...
    lines += ["## How agents use this data", "",
              ...
              "",
              "```",
              "codeatlas query systems                  # this view as JSON",
              "codeatlas query deps <system>            # dependents + dependencies",
              "codeatlas query touches <file-path>      # which system owns this path",
              "codeatlas query system <name>            # Layer 1 + 2 slice for one system",
              "```",
              ""]
    return "\n".join(lines) + "\n"
