...
from codeatlas import SCHEMA_VERSION, paths, boundaries as boundaries_mod
from codeatlas import projects as projects_mod
from codeatlas.stacks import rust as rust_stack
...
def resolve_project(repo_root: Path, project_id: Optional[str] = None) -> dict:
    ...
    if not candidates:
        raise ValueError("no projects detected — run `codeatlas init` to bootstrap")
    if project_id:
        for p in candidates:
            if p["id"] == project_id:
                return p
...
def _load_systems(repo_root: Path, project_id: Optional[str] = None) -> dict:
    ...
    if not p.is_file():
        raise FileNotFoundError(
            f"systems.json not found for project '{_proj['id']}' — run `codeatlas refresh`"
        )
...
def repo_boundaries(repo_root: Path) -> dict:
    ...
    if not bnd_doc:
        return {
            ...
            "note": "No repo-boundaries.json. Run `codeatlas boundaries init --repo` to scaffold one.",
...
def symbol(repo_root: Path, name: str,
    ...
    if stack == "rust":
        ...
        from codeatlas.stacks import python as python_stack
